...
- Make sure you have a new R session
    + `Session` > `Restart R`
- Start a new R script named `IntroVisualisation.R`
- On the first lines enter
...
- There seem to be groups. Are these species? $\implies$ Add colour
...
- Setting the colour in the call to `ggplot()` $\implies$ all geoms will use this
...
- By default, all axes are the same in each facet
