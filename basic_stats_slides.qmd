...
- The baseline terms for the Intercept and `body_mass_g` are now **both** for Adelide
...
- After specifying a 'full' model $\implies$ use `step()` to remove redundant terms
    + Removes terms in a stepwise manner
...
1. Load the `pigs` dataset
2. Make a boxplot:
    + `len` will be the predictor ($y$)
    + Place `dose` on the $x$-axis using `supp` to fill the boxes
3. Using `supp` and `dose` as the predictors and `len` as the response, find the best model
4. Check the residuals and diagnostic plots
5. Make sure you understand the model coefficients
6. What is the 99% confidence interval for `supp = "VC"` & `dose = "High"`
    + What does this mean?
